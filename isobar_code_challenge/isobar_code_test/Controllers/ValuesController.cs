...
namespace isobar_code_test.Controllers
{
    ...
    {
        [HttpPost]
        public IEnumerable<Distance> GetDistancesbetweenAddress([FromBody]string location)
        {
            string path = "~/Data/address list australia.txt";
            Coords locationCoords = HelperExtension.GetLatLngFromAddress(location);
            ...
            foreach(var line in lines)
            {
                ...
                var dist = new Distance(location, line, distance);
                distances.Add(dist);
            }
            return distances.OrderBy(x => x.Distanceinkm);
        }
    }
}
